...
import { createMockRequestHandler, jsonRpcRequest, streamableCard } from "./helpers.js";
...
describe("jsonRpcHandler", () => {
	...
	describe("JSON-RPC routing", () => {
		it("returns method-not-found for unknown method", async () => {
			...
			const res = await app.request("/", jsonRpcRequest("nonexistent/method", {}));
	...
	describe("SSE streaming", () => {
		...
		it("emits SSE error event when stream throws mid-flight", async () => {
			...
			assert(errorData !== undefined, "errorData should be defined");
		...
		it("does not leak internal error details in SSE error events", async () => {
			// Generator yields nothing — throws immediately to simulate a stream error
			// biome-ignore lint/correctness/useYield: intentional throw-only generator for testing
			...
			const errorChunk = text
				.split("\n\n")
				.filter(Boolean)
				.find((c) => c.startsWith("event: error"));
			expect(errorChunk).toBeDefined();
			assert(errorChunk !== undefined, "errorChunk should be defined");
	...
	describe("authentication", () => {
		...
		it("returns 500 when userBuilder throws", async () => {
			...
			const app = new Hono();
			app.post("/", jsonRpcHandler({ requestHandler: createMockRequestHandler(), userBuilder }));
