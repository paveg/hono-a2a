...
import { A2AError, JsonRpcTransportHandler, ServerCallContext } from "@a2a-js/sdk/server";
import type { Handler } from "hono";
import { formatSSEErrorEvent, formatSSEEvent, SSE_HEADERS } from "./sse.js";
import { type JsonRpcHandlerOptions, UserBuilder } from "./types.js";
...
function extensionHeaderValue(context: ServerCallContext): string | undefined {
...
export function jsonRpcHandler(options: JsonRpcHandlerOptions): Handler {
	...
	return async (c) => {
		...
		try {
			...
			return c.json({ jsonrpc: "2.0", id: null, error: error.toJSONRPCError() }, 400);
		}

		try {
			const user = await userBuilder(c);
			const extensions = parseExtensionHeader(c.req.header(HTTP_EXTENSION_HEADER));
			...
			const requestId = (body as { id?: unknown })?.id ?? null;
			return c.json({ jsonrpc: "2.0", id: requestId, error: a2aError.toJSONRPCError() }, 500);
		}
	};
}
